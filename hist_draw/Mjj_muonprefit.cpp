...
void Mjj_muonprefit(){
 ...
 cs -> Print("Mjj_muonprefit.pdf["); // Make blank page in pdf file

 auto hs = new THStack("hs","Mjj_prefit_Muon"); // Object used to plot histograms stacked on one another
...
/*
  TH1D* h_Data   = new TH1D("h_Data" ,"h_Data", 10, 1000, 6000);
   h_Data->SetMarkerStyle(8);

  TH1D* h_EW_Wjj = new TH1D("h_EW_Wjj" ,"h_EW_Wjj", 10, 1000, 6000);
    h_EW_Wjj->SetFillColor(kBlue);
    h_EW_Wjj->SetLineColor(kBlue);

    
  TH1D* h_QCDWjj = new TH1D("h_QCDWjj" ,"h_QCDWjj", 10, 1000, 6000);
    h_QCDWjj->SetFillColor(kRed+1);
    h_QCDWjj->SetLineColor(kRed+1);

  TH1D* h_NonWjj    = new TH1D("h_NonWjj" ,"h_NonWjj", 10, 1000, 6000);
    h_NonWjj->SetFillColor(kGreen+1); 
    h_NonWjj->SetLineColor(kGreen+1); 

  TH1D* h_Multijet    = new TH1D("h_Multijet" ,"h_Multijet", 10, 1000, 6000);
    h_Multijet -> SetFillColor(kViolet+1); 
    h_Multijet -> SetLineColor(kViolet+1);
*/

  datatree->Draw("Mjj>>h_Data", "(nGapJets == 0)&&(cut>=16)&&(Mjj>1000)&&(passReco==1)", "goff");
  ewtree->Draw("Mjj>>h_EW_Wjj", "((nGapJets==0) && (cut>=16) && (Mjj>1000) && (passReco==1))*(finalWeight)*(Lumi)", "goff");
  qcdtree->Draw("Mjj>>h_QCDWjj", "((nGapJets == 0)&&(cut>=16)&&(Mjj>1000)&&(passReco==1))*(finalWeight)*(Lumi)", "goff");
  nonwtree->Draw("Mjj>>h_NonWjj", "((nGapJets == 0)&&(cut>=16)&&(Mjj>1000)&&(passReco==1))*(finalWeight)*(Lumi)", "goff");
  //cout << h_NonWjj->GetEntries() <<endl;
  ...
  //cout << h_Multijet->GetEntries() <<endl;
  ...
  cs -> Update();
  cs -> Print("Mjj_muonprefit.pdf");
  cs -> Print("Mjj_muonprefit.pdf]"); // Need to call this agani to tell the PDF that this was the last page ( thats the "]" )
  cs -> Clear();
