...
void Mjj_elepostfit(){
 TFile* dataf = new TFile("../Ntuples/Data_All_Years_ele_v23.5.root", "read");
 TTree* datatree = (TTree*) dataf->Get("CollectionTree_NOM");

 TFile* ewf = new TFile("../Ntuples/EW_Pow_All_Years_ele_v23.5.root", "read");
 TTree* ewtree = (TTree*) ewf->Get("CollectionTree_NOM");

 TFile* qcdf = new TFile("../Ntuples/QCD_Wjets_Powheg_All_Years_ele_v23.5.root", "read");
 TTree* qcdtree = (TTree*) qcdf->Get("CollectionTree_NOM");

 TFile* nonwf = new TFile("../Ntuples/NonW_All_Years_ele_v23.5.root", "read");
 TTree* nonwtree = (TTree*) nonwf->Get("CollectionTree_NOM");

 TFile* multif = new TFile("../Ntuples/Multijet_All_ele_v23.5.root", "read");
 ...
 cs -> Print("Mjj_elepostfit.pdf["); // Make blank page in pdf file

 auto hs = new THStack("hs","Mjj_postfit_ele"); // Object used to plot histograms stacked on one another
  ...
  ewtree->Draw("Mjj>>h_EW_Wjj", "(nGapJets == 0)*(cut>=16)*(Mjj>1000)*(passReco==1)*(finalWeight)*(Lumi)*(21262.152943/21945.828)", "goff");
  qcdtree->Draw("Mjj>>h_QCDWjj", "(nGapJets == 0)*(cut>=16)*(Mjj>1000)*(passReco==1)*(finalWeight)*(Lumi)*(44357.693598/43964.702)", "goff");
  ...
  cs -> Update();
  cs -> Print("Mjj_elepostfit.pdf");
  cs -> Print("Mjj_elepostfit.pdf]"); // Need to call this agani to tell the PDF that this was the last page ( thats the "]" )
  cs -> Clear();
