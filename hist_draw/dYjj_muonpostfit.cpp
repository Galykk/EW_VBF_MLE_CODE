...
void dYjj_muonpostfit(){
 ...
 cs -> Print("dYjj_muonpostfit.pdf["); // Make blank page in pdf file

 auto hs = new THStack("hs","dYjj_postfit_Muon"); // Object used to plot histograms stacked on one another

   static const Double_t edges[10] = {2.0, 2.6, 3.1, 3.6, 3.9, 4.2, 4.6, 5.0, 6.0, 8.0};
  ...
  ewtree->Draw("dYjj>>h_EW_Wjj", "(nGapJets == 0)*(cut>=16)*(Mjj>1000)*(passReco==1)*(finalWeight)*(Lumi)*(21824.536/21816.659)", "goff");
  qcdtree->Draw("dYjj>>h_QCDWjj", "(nGapJets == 0)*(cut>=16)*(Mjj>1000)*(passReco==1)*(finalWeight)*(Lumi)*(47020.670/51127.686)", "goff");
  ...
  cs -> Update();
  cs -> Print("dYjj_muonpostfit.pdf");
  cs -> Print("dYjj_muonpostfit.pdf]"); // Need to call this agani to tell the PDF that this was the last page ( thats the "]" )
  cs -> Clear();
